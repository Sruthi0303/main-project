...
<td>
    ...
    $principalStatus = strtolower($row['principal_status']);
    $deanStatus = $row['dean_status'];

    if ($role === 'dean' && $principalStatus === 'pending') {
        // Show buttons only for dean-added events with pending principal status
        ?>
        <button class="btn btn-success btn-sm dean-approve-btn" data-event-id="<?= $row['id']; ?>" data-role="principal">Approve</button>
        <button class="btn btn-danger btn-sm dean-reject-btn" data-event-id="<?= $row['id']; ?>" data-role="principal">Reject</button>
        <?php
    } elseif ($role === 'faculty' || $role === 'hod') {
        // Show dean status for faculty or hod events
        echo getStatusBadge($deanStatus);
    } else {
        // Show pending for others
