...
<body>
                ...
                <?php while($row = $pastEventsResult->fetch_assoc()): ?>
                    <tr data-id="<?= $row['id']; ?>">
                        ...
                        <td>
                            ...
                            <button class="btn btn-primary btn-sm" onclick="showUploadForm(<?= $row['id']; ?>)">Upload Photos</button>
                            <form id="upload-form-<?= $row['id']; ?>" style="display:none;" enctype="multipart/form-data">
    ...
    <script>
        // Show upload form for photos
        function showUploadForm(eventId) {
            document.getElementById('upload-form-' + eventId).style.display = 'block';
        }
        ...
        $(document).on('submit', 'form[id^="upload-form-"]', function(event) {
            ...
            $.ajax({
                url: 'upload_event_photo.php',  // Ensure this script handles the photo upload
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
